...
class Screen(object):
    ...
    def build_cmd(self):
        ...
        # set resolution
        if self.is_enabled() and (self.curr_mode.resolution() == self.set.resolution or not self.set.resolution):
            cmd.append('--auto')
        else:
        ...
        if self.set.brightness:
            cmd.extend(['--brightness', str(self.set.brightness)])
            has_changed = True
